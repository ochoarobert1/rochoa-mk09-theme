...
if (!is_admin()) add_action('wp_enqueue_scripts', 'rochoa_jquery_enqueue');
function rochoa_jquery_enqueue() {
...
load_theme_textdomain( 'rochoa', get_template_directory() . '/languages' );
...
register_nav_menus( array(
    'header_menu' => __( 'Menu Header - Principal', 'rochoa' ),
    'footer_menu' => __( 'Menu Footer - Principal', 'rochoa' ),
) );
...
add_action( 'widgets_init', 'rochoa_widgets_init' );
function rochoa_widgets_init() {
    register_sidebar( array(
        'name' => __( 'Sidebar Principal', 'rochoa' ),
        'id' => 'main_sidebar',
        'description' => __( 'Estos widgets seran vistos en las entradas y páginas del sitio', 'rochoa' ),
    ...
    //    register_sidebar( array(
    //        'name' => __( 'Shop Sidebar', 'rochoa' ),
    //        'id' => 'shop_sidebar',
    //        'description' => __( 'Estos widgets seran vistos en Tienda y Categorias de Producto', 'rochoa' ),
    //        'before_widget' => '<li id='%1$s' class='widget %2$s'>',
    //        'after_widget'  => '</li>',
    //        'before_title'  => '<h2 class='widgettitle'>',
    //        'after_title'   => '</h2>',
...
function rochoa_custom_admin_styles() {
    $version_remove = NULL;
...
add_action('login_head', 'rochoa_custom_admin_styles');
add_action('admin_init', 'rochoa_custom_admin_styles');


function rochoa_dashboard_footer() {
    echo '<span id="footer-thankyou">';
    _e ('Gracias por crear con ', 'rochoa' );
    echo '<a href="http://wordpress.org/" target="_blank">WordPress.</a> - ';
    _e ('Tema desarrollado por ', 'rochoa' );
    echo '<a href="http://robertochoa.com.ve/?utm_source=footer_admin&utm_medium=link&utm_content=rochoa" target="_blank">Robert Ochoa</a></span>';
}
add_filter('admin_footer_text', 'rochoa_dashboard_footer');
