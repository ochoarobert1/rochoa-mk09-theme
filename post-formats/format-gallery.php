...
<article id="post-<?php the_ID(); ?>" class="the-single col-9 <?php echo join(' ', get_post_class()); ?>" itemscope itemtype="http://schema.org/Article">
    ...
    <header>
        ...
        <?php the_tags( __( 'Tags: ', 'rochoa' ), ', ', '<br>'); // Separated by commas with a line break at the end ?>
    </header>
    <div class="post-content" itemprop="articleBody">
        <?php the_content() ?>
        <?php wp_link_pages( array(
    'before'      => '<div class="page-links"><span class="page-links-title">' . __( 'Pages:', 'rochoa' ) . '</span>',
    'after'       => '</div>',
    'link_before' => '<span>',
    'link_after'  => '</span>', ) ); ?>
        <footer>
